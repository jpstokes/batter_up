...
describe BatterUp do
  ...
  let!(:player_2) { FactoryGirl.create(:player, :first_name => 'James', :player_id => 2) }
  ...
  describe '#batting_averages' do
    ...
    context 'when within period' do
      ...
      it 'should return correct string' do
        player = ["Name: #{player_1.first_name} #{player_1.last_name}, Batting Avg: #{player_1.batting_average(2000, 2012)}"]
    ...
    context 'when multiple players match criteria' do
      ...
      it 'should return correct string array' do
        players = ["Name: #{player_1.first_name} #{player_1.last_name}, Batting Avg: #{player_1.batting_average(2000, 2012)}"]
        players << "Name: #{player_2.first_name} #{player_2.last_name}, Batting Avg: #{player_2.batting_average(2000, 2012)}"
    ...
    context 'when within year' do
      context 'when on specified team' do
        ...
        it 'returns correct string array' do
          players = ["Name: #{player_1.first_name} #{player_1.last_name}, Slugging Percentage: #{player_1.slugging_percentage(2012, 2012)}"]
  ...
  describe '#triple_crown_winner' do
    ...
    context 'when critieria met' do
      it 'displays the correct string' do
        expect(app.triple_crown_winner('AL', 2012)).to eq('AL Triple Crown Winner for 2012: john doe')
      end
    end

    context 'when criteria not met' do
      it 'displays no winner if no player in league won' do
        expect(app.triple_crown_winner('NL', 2012)).to eq('NL Triple Crown Winner for 2012: (No winner)')
      end

      it 'displays no winner if no player in year won' do
        expect(app.triple_crown_winner('AL', 2013)).to eq('AL Triple Crown Winner for 2013: (No winner)')
      end
    end
  end
end
